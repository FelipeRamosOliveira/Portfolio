...
for i=1:length(elem(:,1))
	...
	%Determine the global element displacements
        ...
        for mn=1:totalm
            dbar(1:2,1)=mode(4*nnodes*(mn-1)+2*nodei-1:4*nnodes*(mn-1)+2*nodei);%+dbar(1:2,1); u1 v1
            dbar(3:4,1)=mode(4*nnodes*(mn-1)+2*nodej-1:4*nnodes*(mn-1)+2*nodej);%+dbar(3:4,1); u2 v2
            dbar(5:6,1)=mode(4*nnodes*(mn-1)+2*nnodes+2*nodei-1:4*nnodes*(mn-1)+2*nnodes+2*nodei);%+dbar(5:6,1);  w1 theta1
            dbar(7:8,1)=mode(4*nnodes*(mn-1)+2*nnodes+2*nodej-1:4*nnodes*(mn-1)+2*nnodes+2*nodej);%+dbar(7:8,1);  w2 theta2
