...
%           dbar(7:8,1)=mode(4*nnodes*(mn-1)+2*nnodes+2*nodej-1:4*nnodes*(mn-1)+2*nnodes+2*nodej);%+dbar(7:8,1);
      ...
      if ifpatch==0
          ...
          if undef==1
             plot3([xi xj],[(k-1)/(km-1)*L (k-1)/(km-1)*L],[zi zj],'y','LineWidth',3)
%              if k==1 & i==1
%                  axis off, axis equal, axis([xmin xmax ymin ymax zmin zmax]), view(37.5,30)
%              end
             hold on
             if k<km
% % % % % % %                 plot3([xi xi],[(k-1)/(km-1)*L (k)/(km-1)*L],[zi zi],'k--','LineWidth',1)
% % % % % % %                 plot3([xj xj],[(k-1)/(km-1)*L (k)/(km-1)*L],[zj zj],'k--','LineWidth',1)
